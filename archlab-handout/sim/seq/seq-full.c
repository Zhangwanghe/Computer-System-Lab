...
long long gen_instr_valid()
{
    return ((icode) == (I_NOP) || (icode) == (I_HALT) || (icode) == 
      ...
      (icode) == (I_MRMOVQ) || (icode) == (I_IADDQ) || (icode) == (I_ALU)
       || (icode) == (I_JMP) || (icode) == (I_CALL) || (icode) == (I_RET)
...
long long gen_need_valC()
{
    return ((icode) == (I_IRMOVQ) || (icode) == (I_RMMOVQ) || (icode) == 
      (I_MRMOVQ) || (icode) == (I_IADDQ) || (icode) == (I_JMP) || (icode)
       == (I_CALL));
}
...
long long gen_dstE()
{
    return ((((icode) == (I_RRMOVQ)) & (cond)) ? (rb) : ((icode) == 
        (I_IRMOVQ) || (icode) == (I_IADDQ) || (icode) == (I_ALU)) ? (rb) : 
